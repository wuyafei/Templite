    ...
    def __init__(self, text, *contexts):
        ...
        def _expr_code(self, expr):
            ...
            if '|' in expr:
                ...
                args = ', '.join(repr(d) for d in dots[1:])  # repr可以确保下面的arg1，arg2上的引号
                code = 'do_dots(%s, %s)' % (code, args)  # 转成形如 do_dots(name, 'arg1', 'arg2', ...)的函数调用
            else:
                code = 'c_%s' % expr
        ...
        for token in tokens:
            if token.startswith('{#'):  # 注释标签
                ...
                if words[0] == 'if':
                    if len(words) != 2:
                        self._syntax_error("wrong format of if expression", token)
                    ops_stack.append('if')
                    code.add_line('if (%s):' % self._expr_code(words[1]))
                    code.indent()
                elif words[0] == 'for':
                    if len(words) != 4 or words[2] != 'in':
                        self._syntax_error('wrong format of for expression', token)
                    ...
                    if len(words) != 1:
                        self._syntax_error('wrong format of end expression', token)
                    end_what = words[0][3:]
                    if not ops_stack:
                        self._syntax_error('end expression not match any if or for', token)
